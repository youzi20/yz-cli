...
export default function Deposit() {
  ...
  const { balance, allowance, approve, receiveTokenPayment } = useDeposit(token.value, amount);

  const isDisabled = !amount || Number(amount) <= 0 || Number(balance) - Number(amount) < 0;
  const isAllowance = Number(allowance) === 0 || Number(allowance) - Number(amount) < 0;
  ...
  return (
    <div className="grid gap-4 p-4">
      ...
      <Button className="mt-4" size="large" colors="secondary" disabled={isDisabled} onClick={handleConfirm}>
        {isAllowance ? <Trans>Approve</Trans> : <Trans>Confirm</Trans>}
      </Button>
    </div>
  );
}
